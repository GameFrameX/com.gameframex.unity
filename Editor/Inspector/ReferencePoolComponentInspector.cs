...
namespace GameFrameX.Editor
{
    ...
    internal sealed class ReferencePoolComponentInspector : GameFrameworkInspector
    {
                ...
                foreach (KeyValuePair<string, List<ReferencePoolInfo>> assemblyReferencePoolInfo in m_ReferencePoolInfos)
                {
                    ...
                    if (currentState)
                    {
                        EditorGUILayout.BeginVertical("box");
                        {
                            ...
                            {
                                string exportFileName = EditorUtility.SaveFilePanel("Export CSV Data", string.Empty, GameFrameworkText.Format("Reference Pool Data - {0}.csv", assemblyReferencePoolInfo.Key), string.Empty);
        ...
        {
            EditorGUILayout.LabelField(m_ShowFullClassName ? referencePoolInfo.Type.FullName : referencePoolInfo.Type.Name, GameFrameworkText.Format("{0}\t{1}\t{2}\t{3}\t{4}\t{5}", referencePoolInfo.UnusedReferenceCount, referencePoolInfo.UsingReferenceCount, referencePoolInfo.AcquireReferenceCount, referencePoolInfo.ReleaseReferenceCount, referencePoolInfo.AddReferenceCount, referencePoolInfo.RemoveReferenceCount));
        }
